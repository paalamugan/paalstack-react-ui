...
```css
/* 1. Library styles (tokens, reset, theme, utilities, fonts, toast) */
...
> **Single-import shortcut:** `@import '@paalstack/react-ui/all.css';` bundles `base.css` +
> `theme.css` + `utilities.css` + `toast.css` + `fonts.css` into one file. Replace it with the
> individual subpaths when you need fine-grained cascade control.
...
@layer base {
  :root {
    ...
    /* Override individual --shadow-* tokens (or the whole scale) */
