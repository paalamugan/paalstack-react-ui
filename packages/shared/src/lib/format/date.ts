...
class DateTransformer implements IDateTransformer<BaseDateType> {
  parse(value: BaseDateType, timeZone: string): Date {
    return toZonedTime(value, timeZone);
  }

  convert(value: Date, timeZone: string): string {
...
export interface FormatOptions<D extends BaseDateType> extends Partial<
  Omit<IDateOptions<D>, 'transformer' | 'dateTimeFormat'>
> {}
...
export class DateIntl<D extends BaseDateType> {
   ...
   * @param options - The options for formatting the date. Can be a string or an object. If a string is provided, it will be used as the date format. If an object is provided, it will be used as the options for formatting the date.
   * @returns The formatted date string.
   */
  format(value: D | undefined | null, options?: FormatOptions<D> | string): string {
    if (typeof options === 'string') {
      if (!this.isValid(value)) return this.fallback;
      return formatInTimeZone(this.toDate(value), this.timeZone, options, { locale: this.locale });
    }
    const {
   ...
   * @returns The formatted date string.
   */
  formatDate(value: D | undefined | null, options?: FormatOptions<D> | string): string {
   ...
   * @param value - The date value to format.
   * @param options - The options for formatting the date and time. Can be a string or an object. If a string is provided, it will be used as the date format. If an object is provided, it will be used as the options for formatting the date and time.
   * @returns The formatted date and time string.
   */
  formatDateTime(value: D | undefined | null, options?: FormatOptions<D> | string): string {
    if (typeof options === 'string') {
      return this.format(value, options);
    }
...
const defaultOptions = getLocalCountryBasedFormat();

export const DEFAULT_DATE_OPTIONS: IDateOptions = {
  dateFormat: defaultOptions.dateFormat,
  dateTimeFormat: defaultOptions.dateTimeFormat,
