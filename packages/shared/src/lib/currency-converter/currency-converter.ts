...
export class CurrencyConverter implements ICurrencyConverter {
  ...
  from(currencyFrom: CurrencyCode) {
    ...
    if (!this.currencyCodes.includes(currencyFrom)) throw new Error(`${currencyFrom} is not a valid currency code`);
  ...
  to(currencyTo: CurrencyCode) {
    ...
    if (!this.currencyCodes.includes(currencyTo)) throw new Error(`${currencyTo} is not a valid currency code`);
  ...
  setupRatesCache(ratesCacheOptions: RatesCacheOptions) {
    ...
    if (!ratesCacheOptions.ratesCacheDuration)
  ...
  async rates() {
    ...
    const { load } = await import('cheerio'); // Lazy load cheerio to avoid bundling it if not needed
  ...
  getCurrencyName(currencyCode: CurrencyCode) {
    ...
    if (!this.currencyCodes.includes(currencyCode)) throw new Error(`${currencyCode} is not a valid currency code`);
