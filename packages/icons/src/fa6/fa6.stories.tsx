...
import * as Fa6Icons from './index';
import { FaHeart, FaHouse, FaStar } from './index';
...
function getIconDisplayName(iconName: string): string {
  const nameWithoutPrefix = iconName.replace(/^[A-Z][a-z]+[0-9]*/, '');
...
export function FontAwesome6() {
  const icons = React.useMemo(() => toIconEntries(Fa6Icons), []);
  ...
  return (
    <VStack className="gap-6">
      <Input placeholder="Search Font Awesome 6 icons..." onChange={onValueChange} />
      <VStack className="mb-6">
        <Heading as="h2">Font Awesome 6 ({filteredIcons.length})</Heading>
