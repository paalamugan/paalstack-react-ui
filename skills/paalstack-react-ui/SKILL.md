...
The skill itself ships inside the npm tarballs (ui: SKILL.md + references +
scripts + README; hooks/icons: SKILL.md + reference) and a `paalstack-skill`
bin entry installs it into agents (`npx paalstack-skill`) in consumer
projects. Update the skill in the same PR as component changes — it is
